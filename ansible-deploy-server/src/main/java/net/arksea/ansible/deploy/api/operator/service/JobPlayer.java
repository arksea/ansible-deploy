...
public class JobPlayer extends AbstractActor {
    private final static Logger logger = LogManager.getLogger(JobPlayer.class);
    ...
    private JobPlayer(OperationJob job, Set<Long>hosts, Set<OperationVariable> operationVariables, JobResources beans) {
        ...
        this.beans = beans;
        Optional<AppOperation> optOp = beans.appOperationDao.findById(job.getOperationId());
        if (optOp.isPresent()) {
            this.operation = optOp.get();
        } else {
            throw new ServiceException("Operation not found");
        }
        Optional<App> optApp = beans.appDao.findById(job.getAppId());
        if (optApp.isPresent()) {
            this.app = optApp.get();
        } else {
    ...
    private void sendJobNotificationMail() {
        try {
            if (job.getTriggerId() != null) {
                Optional<OperationTrigger> op = beans.operationTriggerDao.findById(job.getTriggerId());
                if (op.isPresent() && StringUtils.isNotBlank(op.get().getNotifyEmails())) {
                    OperationTrigger t = op.get();
