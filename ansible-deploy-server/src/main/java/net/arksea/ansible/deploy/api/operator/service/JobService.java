...
public class JobService {
    ...
    public Future<JobPlayer.PollLogsResult> pollJobLogs(long jobId, int index) {
        Optional<OperationJob> jobOp = operationJobDao.findById(jobId);
        if (jobOp.isPresent() && jobOp.get().getEndTime() == null) {
            String path = "akka.tcp://system@" + jobOp.get().getExecHost() + ":" + systemBindPort + "/user/" + makeJobActorName(jobId);
